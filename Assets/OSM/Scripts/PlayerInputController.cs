...
public class PlayerInputController : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        // 키보드(WASD) 입력과 조이패드 입력 합성해서 사용
        Vector2 totalInput = moveInput;
        if (joypadInput != Vector2.zero) totalInput = joypadInput;
    ...
    public void SetJoypadInput(string direction)
    {
        ...
        {
            case "UP":
                joypadInput = Vector2.up;
                break;
            case "DOWN":
                joypadInput = Vector2.down;
                break;
            case "LEFT":
                joypadInput = Vector2.left;
                break;
            case "RIGHT":
                joypadInput = Vector2.right;
                break;
            case "STOP":
                joypadInput = Vector2.zero;
                break;
                // 필요시 다른 버튼도 추가
        }
    }
